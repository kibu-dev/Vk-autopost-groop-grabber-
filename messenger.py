...
from ai_poster import generate_variants, parse_variants, load_prompt, ai_log, generate_text
    ...
    for event in longpoll.listen():
        ...
        if is_admin and user_id in admin_state:
                                ...
                                if sizes:
                                    ...
                                    if photo_url:
                                        ...
                                        if 'photo' in up and up['photo']:
                                            ...
                                            if saved:
                                                photo = saved[0]
                                                set_horoscope_photo(f"photo{photo['owner_id']}_{photo['id']}")
                                                photo_saved = True
                                        break
                except Exception as e:
            ...
            if mode == "holiday_post":
                                    ...
                                    if photo_url:
                                        ...
                                        if 'photo' in up and up['photo']:
                                            ...
                                            if saved:
                                                ...
                                                config["photo_id"] = f"photo{photo['owner_id']}_{photo['id']}"
                ...
                if photo_saved:
                    ...
                    if text:
                        ...
                        save_holidays_config(config)
                        send_message(vk, user_id, f"🤖 Готовое поздравление:\n\n{text[:1500]}", get_holiday_confirm_keyboard())
                    else:
            ...
            if mode == "add_donor":
                ...
                add_forbidden_word(text.strip().lower()); send_message(vk, user_id, "✅ Добавлено!", get_forbidden_words_keyboard())
            elif mode == "del_word":
                w = text.strip().lower()
        ...
        if t in ["начать", "меню", "start"]:
            ...
            waiting_support.add(user_id); send_message(vk, user_id, "📝 Пишите:", get_cancel_keyboard())
        elif t == "🔙 отмена":
            send_message(vk, user_id, "Меню:", get_admin_main_keyboard() if is_admin else get_main_keyboard())
        elif t == "❌ нет":
            selected_post.pop(user_id, None); send_message(vk, user_id, "Отменено.", get_main_keyboard())
        elif t == "✅ да, удалить" and user_id in selected_post:
            pid = selected_post[user_id]
            if get_post_author(pid) == user_id:
                try:
                    vk_user.wall.delete(owner_id=-GROUP_ID, post_id=pid); delete_user_post(user_id, pid)
            ...
            if t in ["🔙 назад в админку", "🔙 назад"]:
                admin_state.pop(user_id, None); send_message(vk, user_id, "Админ-меню:", get_admin_main_keyboard())
                ...
                if scheduled:
                    msg = "📅 Запланированные:\n\n"
                    for p in scheduled[:10]: msg += f"• {datetime.fromtimestamp(p['time']).strftime('%d.%m %H:%M')} — {p['text'][:50]}...\n"
                ...
                ids = list(pending.keys()); idx = 0; d = pending[ids[idx]]
                ...
                if d.get('translated'): msg += "🌐 Переведено\n"
                ...
                drafts = load_drafts(); reddit_pending = len({k: v for k, v in drafts.items() if v.get("status") == "pending"})
                ...
                if donors:
                    lines = [f"• {g} — {get_group_name(vk_user, g)}" for g in donors]
                ...
                send_message(vk, user_id, f"❤️ Автолайкер: {'Включен ✅' if is_liker_enabled() else 'Выключен ❌'}", get_liker_keyboard())
                ...
                if groups:
                    lines = [f"• {g} — {get_group_name(vk_user, g)}" for g in groups]
                    send_message(vk, user_id, "\n".join(lines), get_liker_keyboard())
                else: send_message(vk, user_id, "📭 Пусто.", get_liker_keyboard())
                ...
                send_message(vk, user_id, f"🟢 Онлайн: {'Включен ✅' if is_online_enabled() else 'Выключен ❌'}", get_online_keyboard())
            ...
            elif t == "▶️ включить группу": set_group_accept_enabled(True); send_message(vk, user_id, "👥 Автоприём в группу включен!", get_group_accept_keyboard())
            elif t == "⏸️ выключить группу": set_group_accept_enabled(False); send_message(vk, user_id, "👥 Автоприём в группу выключен.", get_group_accept_keyboard())
            elif t == "🔮 гороскоп":
                ...
                if create_horoscope():
                    config = load_json("horoscope_config.json", {})
                    next_m = config.get("next_monday", ""); next_str = datetime.fromisoformat(next_m).strftime("%d.%m %H:%M") if next_m else "не запланирован"
                    msg = f"✅ Готово!\n🔮 Гороскоп: {'Включен ✅' if get_horoscope_enabled() else 'Выключен ❌'}\nСледующий: {next_str}"
                    if config.get("text"): msg += f"\n\n📝 Текст:\n{config['text'][:2500]}"
                    send_message(vk, user_id, msg, get_horoscope_keyboard())
                else: send_message(vk, user_id, "❌ Ошибка создания.", get_horoscope_keyboard())
            elif t == "▶️ включить гороскоп": set_horoscope_enabled(True); send_message(vk, user_id, "🔮 Гороскоп включен!", get_horoscope_keyboard())
            elif t == "⏸️ выключить гороскоп": set_horoscope_enabled(False); send_message(vk, user_id, "🔮 Гороскоп выключен.", get_horoscope_keyboard())
            ...
            elif t == "🖼️ сменить фото": admin_state[user_id] = {"mode": "horoscope_photo"}; send_message(vk, user_id, "📷 Пришлите новое фото для гороскопа:", get_cancel_keyboard())
            elif t == "🎉 праздники":
                config = get_holidays_config()
                if not config.get("holidays_list"):
                    send_message(vk, user_id, "⏳ Загружаю список праздников через ИИ...")
                ...
                else: send_message(vk, user_id, "📭 Список пуст.", get_holidays_keyboard())
            elif t == "➡️ следующий":
                config = get_holidays_config(); holidays = config.get("holidays_list", [])
                if holidays:
                    idx = (config.get("current_index", 0) + 1) % len(holidays); config["current_index"] = idx
                ...
                admin_state[user_id] = {"mode": "holiday_post"}; send_message(vk, user_id, f"📷 Пришлите фото для поздравления\nПраздник: {name}", get_cancel_keyboard())
                ...
                if create_holiday_post(vk_user): send_message(vk, user_id, f"✅ Поздравление запланировано!\n📅 {date_str} 10:00", get_admin_main_keyboard())
                ...
                if t == "⬅️ предыдущий": idx = (idx - 1) % len(ids)
                    ...
                    for img_url in d.get('images', [])[:10]:
                        try:
                            ...
                            upload_server = vk_user.photos.getWallUploadServer(group_id=GROUP_ID)
                            files = {'photo': ('reddit.jpg', resp.content, 'image/jpeg')}
                            up = req.post(upload_server['upload_url'], files=files).json()
                            if 'photo' in up and up['photo']:
                                saved = vk_user.photos.saveWallPhoto(photo=up['photo'], server=up['server'], hash=up['hash'], group_id=GROUP_ID)
                    ...
                    if original:
                        ...
                        translated = generate_text(f"Переведи на русский язык:\n\n{original[:2000]}")
                        if translated:
                            drafts[draft_id]["text"] = translated; drafts[draft_id]["translated"] = True; save_drafts(drafts)
                            pending = {k: v for k, v in drafts.items() if v.get("status") == "pending"}; d = pending[ids[idx]]
                    ...
                    admin_state.pop(user_id, None); continue
                elif t == "✏️ редактировать": admin_state[user_id] = {"mode": "reddit_edit", "draft_id": ids[idx]}; send_message(vk, user_id, "✏️ Напишите новый текст:", get_cancel_keyboard()); continue
                elif t == "❌ удалить": del drafts[ids[idx]]; save_drafts(drafts); send_message(vk, user_id, "🗑 Удалён.", get_admin_main_keyboard()); admin_state.pop(user_id, None); continue
