        ...
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            if is_admin and user_id in admin_state:
                ...
                if mode == "reddit_view":
                    ...
                    if t == "⬅️ назад": idx = (idx - 1) % len(ids)
                        ...
                        if d.get('images'):
                            send_message(vk, user_id, "⏳ Загружаю фото...", get_reddit_post_keyboard(bool(d.get('text', '').strip()), bool(d.get('title', '').strip())))
                        ...
                        if not attachments:
                            send_message(vk, user_id, "❌ Не удалось загрузить фото.", get_reddit_post_keyboard(bool(d.get('text', '').strip()), bool(d.get('title', '').strip())))
                            continue
                ...
                if mode == "ai_post":
                    ...
                    if result:
                        ...
                        else: send_message(vk, user_id, "❌ Не удалось.", get_admin_main_keyboard()); admin_state.pop(user_id, None)
                    else: send_message(vk, user_id, "❌ Ошибка ИИ.", get_admin_main_keyboard()); admin_state.pop(user_id, None)
                    continue
                ...
                if mode == "ai_choose":
                    if t == "✅ опубликовать":
                        ...
                        add_published_post(r["post_id"], ADMIN_ID, chosen); admin_state.pop(user_id, None)
                        ...
                        if result:
                            variants = parse_variants(result)
                            if variants and len(variants[0]) > 20: state["variants"] = variants; send_message(vk, user_id, f"🤖 Новый вариант:\n\n{variants[0]}", get_variants_keyboard())
                ...
                if mode == "add_donor":
                    ...
                    if gid: add_donor_group(gid); send_message(vk, user_id, f"✅ [{get_group_name(vk, gid)}] добавлена!", get_donor_groups_keyboard())
            ...
            if t in ["начать", "меню", "start"]:
                ...
                if get_post_author(pid) == user_id:
                    try: vk.wall.delete(owner_id=-GROUP_ID, post_id=pid); delete_user_post(user_id, pid); send_message(vk, user_id, f"✅ #{pid} удалён!", get_main_keyboard())
                    except: send_message(vk, user_id, "❌ Ошибка.", get_main_keyboard())
                ...
                if t in ["🔙 назад в админку", "🔙 назад"]: admin_state.pop(user_id, None); send_message(vk, user_id, "Админ-меню:", get_admin_main_keyboard())
                    ...
                    donors = get_donor_groups()
                    send_message(vk, user_id, "Выберите:" if donors else "📭 Пусто.", get_remove_donor_keyboard(donors, vk) if donors else get_donor_groups_keyboard())
                elif t.startswith("➖ "):
                    donors = get_donor_groups()
                    for g in donors:
                        try: name = get_group_name(vk, g)
                        except: name = str(g)
                    ...
                    pid = int(t.split()[-1])
                    try:
                        posts = vk.wall.get(owner_id=-GROUP_ID, filter="suggests", count=100)
                        for p in posts.get("items", []):
                            if p["id"] == pid:
                                pt = p.get("text", ""); puid = p.get("from_id", 0)
                                fn = f"{pt}\n\nАвтор: Аноним" if contains_anonymous(pt) else f"{pt}\n\nАвтор: [id{puid}|{get_user_name(vk, puid)[0]} {get_user_name(vk, puid)[1]}]"
                                r = vk.wall.post(owner_id=-GROUP_ID, message=fn, attachments=build_attachments(p), from_group=1)
                                vk.wall.delete(owner_id=-GROUP_ID, post_id=pid); add_published_post(r["post_id"], puid, pt); remove_from_moderation(pid)
                    ...
                    try:
                        ...
                        if 0 <= idx < len(pending):
                            ...
                            vk.wall.post(owner_id=-GROUP_ID, message=p["post"]["text"], attachments=p["post"]["attachments"], from_group=1, publish_date=pub_time)
                    ...
                    pid = int(t.split()[-1])
                    try: vk.wall.delete(owner_id=-GROUP_ID, post_id=pid)
                    except: pass
                    ...
                    if create_holiday_post(vk): send_message(vk, user_id, f"✅ Запланировано!\n📅 {date_str} 10:00", get_admin_main_keyboard())
