    ...
    for event in longpoll.listen():
        ...
        if is_admin and user_id in admin_state:
            ...
            if mode == "add_donor":
                ...
                send_message(vk, user_id, f"✅ Добавлено!", get_forbidden_words_keyboard())
            elif mode == "del_word":
                w = text.strip().lower()
        ...
        # ─── АДМИН ───
            ...
            if t in ["🔙 назад в админку", "🔙 назад"]:
                ...
                pid = int(t.split()[-1])
                remove_from_moderation(pid)
                send_message(vk, user_id, "⏭ Пропущен.", get_admin_main_keyboard())
        else:
